...
import { ApplicationStatus } from './main/application-status';
...
const createWindow = () => {
  ...
  // appStatus.ListenForStatus("Notepad", "Untitled - Notepad", (status) => {
  //   console.log("ApplicationStatus callback:", status);
  // });
};

app.on('before-quit', () => {
  ApplicationStatus.getInstance().StopListening();
  console.log("ApplicationStatus stopped listening before quit.");
});
